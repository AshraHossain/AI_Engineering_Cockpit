...
The cloud half of the hybrid orchestrator: always available (given an API
key), used both as the default backend on platforms without Ollama
(Windows) and as the fallback when local generation isn't available.

Uses the current ``google-genai`` SDK, where a single ``genai.Client`` is
constructed once and the model id is supplied per-call via
``client.models.generate_content(model=..., contents=...)``. The legacy
``google-generativeai`` package (``genai.configure()`` plus a per-model
``GenerativeModel`` object) is end-of-life and no longer used.
"""
...
class CloudGeminiError(RuntimeError):
    """Raised when the cloud Gemini client cannot produce a response."""
...
class CloudGeminiClient:
    ...
    Raises:
        CloudGeminiError: If no API key is available and no ``client``
            was injected.
    """
    ...
    def __init__(
        ...
        if self._client is None:
            ...
            if not resolved_key:
                raise CloudGeminiError("GEMINI_API_KEY is not set and no client was injected.")
    ...
    def generate(self, prompt: str) -> str:
        ...
        Raises:
            CloudGeminiError: If the prompt is empty or the SDK call
                fails / returns no usable text.
        """
        if not prompt or not prompt.strip():
            raise CloudGeminiError("Prompt must be a non-empty string.")
        ...
        try:
            ...
            raise CloudGeminiError(f"Gemini request failed: {exc}") from exc

        text = getattr(response, "text", None)
        if not text:
            raise CloudGeminiError("Gemini response contained no text.")
